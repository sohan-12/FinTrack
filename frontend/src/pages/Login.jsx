...
import { Flame, Lock, Mail, ArrowRight, X, PlusCircle, UserCheck, Shield } from 'lucide-react';
...
export const Login = () => {
  ...
  const handleGoogleSignIn = async (googleAccount) => {
    try {
      ...
      const loggedInUser = await googleLogin(googleAccount);
      if (loggedInUser.role === 'ADMIN') {
        navigate('/admin', { replace: true });
      } else {
        const from = location.state?.from?.pathname || '/dashboard';
  ...
  return (
    ...
    >
      ...
      {/* Google OAuth One-Tap Dialog Modal */}
      {isGoogleModalOpen && (
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '440px', padding: '28px 24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <h3 style={{ fontWeight: 800, fontSize: '1.2rem', color: '#0F172A' }}>Sign in with Google</h3>
              </div>
            ...
            {!isCustomGoogleMode ? (
              <>
                <p style={{ fontSize: '0.85rem', color: '#64748B', marginBottom: '16px' }}>
                  Choose an account to continue to <strong>FinTrack</strong>:
                </p>
                ...
                {/* Option to enter any other email */}
                <button
                  type="button"
              ...
              <form onSubmit={handleCustomGoogleSubmit}>
                <p style={{ fontSize: '0.85rem', color: '#475569', marginBottom: '14px', lineHeight: 1.5 }}>
                  Enter your Google Account email. FinTrack will create and authenticate your account.
                </p>
